...
class Stage extends Container {
  ...
  setBackground(backgroundName) {
    const backgroundTexture =
      loader.resources[this.spritesheet].textures[
        `scene/backgrounds/${backgroundName}`
      ];
    ...
    // Add the new background and keep a reference to it
  ...
  _setStage() {
    // const backgroundTexture =
    //   loader.resources[this.spritesheet].textures[
    //     "scene/backgrounds/redbg.jpg"
    //   ];
    // const background = new PIXI.Sprite(backgroundTexture);
    // background.position.set(0, 0);

    const treeTexture =
      loader.resources[this.spritesheet].textures["scene/tree/0.png"];
    const tree = new PIXI.Sprite(treeTexture);
    tree.position.set(100, 237);

    // If 'foreignBg' is another background element, ensure it's above the main background
    const foreignBgTexture =
      loader.resources[this.spritesheet].textures["scene/back/0.png"];
    const foreignBg = new PIXI.Sprite(foreignBgTexture);
    foreignBg.position.set(0, 0);

    // this.addChildAt(background, 0);
    this.addChildAt(tree, 1);
    this.addChildAt(foreignBg, 2);
    this.addChildAt(this.dog, 3);
    this.addChildAt(this.flashScreen, 4);
    this.addChildAt(this.hud, 5);
